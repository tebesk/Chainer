...
i=0
for filename in random.sample(Ansfiles,30):
	print(filename)
	train_image = chainer.Variable(np.asarray([[cv2.imread("denoised_training_data/"+filename, 0)/255.0]], dtype=np.float32))
	target = chainer.Variable(np.asarray([[cv2.imread("ans_area/"+filename, 0)/255.0]], dtype=np.float32))	
...
for filename in Ansfiles:
	train_image = chainer.Variable(np.asarray([[cv2.imread("denoised_training_data/"+filename, 0)/255.0]], dtype=np.float32))
	trained = model.forward(train_image).data[0][0]*255
	cv2.imwrite("160802_area/1st/"+filename, trained)
